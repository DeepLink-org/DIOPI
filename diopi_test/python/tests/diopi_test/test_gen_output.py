...
case_cfg_map = {
    ...
    "test backward": {
        "batch_norm::batch_norm_0.pth": {
            "name": "batch_norm",
            "atol": 0.001,
            "rtol": 0.0001,
            "atol_half": 0.1,
            "rtol_half": 0.01,
            "para": {"training": False, "momentum": 0.1, "eps": 1e-05},
            "tensor_para": {
                "args": [
                    {
                        "ins": "input",
    ...
    "test no_output_ref": {
        "dropout::dropout_0.pth": {
            "name": "dropout",
            "no_output_ref": True,
            "is_inplace": True,
            "para": {"p": 0.5, "training": True},
            "tensor_para": {
                "args": [
                    {
                        "ins": "input",
    ...
    "test CustomizedTest": {
        "clip_grad_norm::clip_grad_norm__0.pth": {
            ...
            "para": {"max_norm": 5, "norm_type": 3.0, "error_if_nonfinite": False},
            "tensor_para": {
                "args": [
                    {
                        "ins": "tensors",
    ...
    "test dtype in kwargs": {
        "prod::prod_0.pth": {
            "name": "prod",
            "interface": ["torch"],
            "atol_half": 0.0001,
            "rtol_half": 0.001,
            "para": {"dim": 0, "dtype": np.bool_},
            "tensor_para": {
                "args": [
                    {
                        "ins": "input",
                        "shape": (2, 80, 128, 128, 1),
                        "dtype": np.float32,
                        "gen_fn": "Genfunc.randn",
                        "gen_policy": "default",
                        "requires_grad": [False],
    ...
    "test cat requires_grad": {
        "cat_diff_size::cat_14.pth": {
            "atol": 0.0001,
            "rtol": 1e-05,
            "atol_half": 0.01,
            "rtol_half": 0.05,
                ...
                "args": [
                    {
                        "ins": "tensors",
                        "requires_grad": [True],
                        "shape": ((2, 31, 0), (2, 31, 512), (2, 31, 128)),
                        "dtype": np.float64,
    ...
    "test inplace requires_grad": {
        "unfold::unfold_0.pth": {
            ...
            "name": "unfold",
            "interface": ["torch.Tensor"],
            "para": {"dimension": 2, "size": 2, "step": 1},
            "tensor_para": {
                "gen_fn": "Genfunc.randn",
                "args": [
                    {
    ...
    "test output is tuple": {
        "cholesky::cholesky_ex_1.pth": {
            "atol": 1e-05,
            "rtol": 1e-05,
            "atol_half": 0.01,
            "rtol_half": 0.05,
            ...
            "para": {"upper": False, "check_errors": False},
            "tensor_para": {
                "args": [
                    {
                        "ins": "input",
...
class TestGenOutputData(object):
        ...
        for case_name in case_cfg:
            ...
            if_backward = (
                len(
                    [
                        i["requires_grad"]
                        for i in case_cfg[case_name]["tensor_para"]["args"]
            ...
            if if_backward:
                os.remove(
                    os.path.join(
                        outputs_path, case_name.split(".pth")[0] + "_backward.pth"
                    )
                )
        ...
        for case_name in case_cfg:
            ...
            if case_cfg[case_name].get("no_output_ref", False):
                assert not os.path.exists(
                    case_path
                ), f"Expect file {case_path} not found"
                continue
            ...
            saved_backward_pth = os.path.join(
                outputs_path, case_name.split(".pth")[0] + "_backward.pth"
            )
            if_backward = (
                len(
            ...
            if if_backward:
                assert os.path.exists(
                    saved_backward_pth
                ), f"File {saved_backward_pth} not found"
    ...
    def check_output(self, cfg, forward_result, backward_result):
        ...
        if backward_result is not None:
            grad_key = [
                i["ins"]
                for i in cfg["tensor_para"]["args"]
                if i["requires_grad"] == [True]
            ]
            assert len(grad_key) == len(
                backward_result
            ), f"expect grad output's length is {len(grad_key)}, but got {len(backward_result)}"
            for key in grad_key:
                assert (
                    key in backward_result
                ), f'generate {cfg["name"]} grad for {key} failed'
