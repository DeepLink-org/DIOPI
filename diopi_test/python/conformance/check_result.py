import numpy as np
from diopilib.diopirt import Tensor, InputChangedException, OutputCheckFailedException
from conformance.utils import glob_vars

class CheckResult(object):
    @staticmethod
    def compare_input(input1: dict, input2: dict, ignore_paras_for_input_check: set):
        ...
        for name, value in input1.items():
            assert name in input2.keys(), f"{name} not exist in input2"
    ...
    def compare_output(output, output_reference, **kwargs):
        ...
        compare_fn(output.numpy(), output_reference, kwargs)
