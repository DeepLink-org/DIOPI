...
class DeviceConfig(object):
    ...
    def _collect_options(self, key, value):
        ...
        if key in self._device_rules.keys():
            raise AttributeError(f"Device Config Error for {key}: config {key} many times.")
        if 'name' not in value.keys():
            raise AttributeError(f"Device Config Error for {key}: config {key} has not name.")
        ...
        if 'tensor_para' in value.keys() and 'args' in value['tensor_para'].keys():
            ...
            self._device_rules[cfg_name]['skip']['tensor_para'] = {} # no need args for filter rule
            ...
            for ins in expand_arg_list:
                ins_key = ins['ins']
                self._device_rules[cfg_name]['skip']['tensor_para'][ins_key] = {} # for each args, i.e. input
                for k, v in ins.items():
                    if k != 'ins':
                        self._device_rules[cfg_name]['skip']['tensor_para'][ins_key][k] = set()
                        for sk in v:
                            if isinstance(sk, Skip):
                                self._device_rules[cfg_name]['skip']['tensor_para'][ins_key][k].add(sk.value())
    # @staticmethod
    def run(self):
            ...
            if 'para' in rule['skip'].keys() and 'para' in case_cfg.keys():
                ...
                for pk, pv in para_case.items():
                    if pk in para_rule.keys() and pv in para_rule[pk]:
                        return True
            if 'tensor_para' in rule['skip'].keys() and 'tensor_para' in case_cfg.keys():
                # 'tensor_para
