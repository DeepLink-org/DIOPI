...
class Storage final {
 ...
 public:
  Storage(malloc_func_t mallocFn, free_func_t freeFn, int64_t nbytes)
      : mallocFn_(mallocFn), freeFn_(freeFn), nbytes_(nbytes) {
    assert(freeFn_);
    assert(mallocFn_);
...
struct diopiTensor {
 ...
 public:
  diopiTensor(const diopiSize_t* shape,
...
diopiTensor::diopiTensor(const diopiSize_t* shape,
                         const diopiSize_t* stride,
                         diopiDtype_t dtype,
                         diopiDevice_t device,
                         diopiContextHandle_t context) {
...
DIOPI_RT_API diopiError_t diopiGetTensorData(diopiTensorHandle_t th,
                                             void** pptr) {
  *pptr = th->data();
  return diopiSuccess;
}

DIOPI_RT_API diopiError_t diopiGetTensorDataConst(diopiConstTensorHandle_t th,
                                                  const void** pptr) {
  *pptr = th->data();
  return diopiSuccess;
}

DIOPI_RT_API diopiError_t diopiGetTensorShape(diopiConstTensorHandle_t th,
                                              diopiSize_t* size) {
  *size = th->shape();
  return diopiSuccess;
}

DIOPI_RT_API diopiError_t diopiGetTensorStride(diopiConstTensorHandle_t th,
                                               diopiSize_t* stride) {
  *stride = th->stride();
  return diopiSuccess;
}

DIOPI_RT_API diopiError_t diopiGetTensorDtype(diopiConstTensorHandle_t th,
                                              diopiDtype_t* dtype) {
  *dtype = th->dtype();
  return diopiSuccess;
}

DIOPI_RT_API diopiError_t diopiGetTensorDevice(diopiConstTensorHandle_t th,
                                               diopiDevice_t* device) {
  *device = th->device();
  return diopiSuccess;
}

DIOPI_RT_API diopiError_t diopiGetTensorNumel(diopiConstTensorHandle_t th,
                                              int64_t* numel) {
  *numel = th->numel();
  return diopiSuccess;
}
...
DIOPI_RT_API diopiError_t diopiTensorResetShape(diopiTensorHandle_t th,
...
DIOPI_RT_API diopiError_t diopiTensorGetCtxHandle(diopiConstTensorHandle_t th,
...
struct diopiContext {
 ...
 public:
  ...
  diopiTensorHandle_t createTensor(const diopiSize_t* size,
                                   const diopiSize_t* stride,
                                   const diopiDtype_t dtype,
                                   const diopiDevice_t dev) {
    diopiTensorHandle_t tensor =
        new diopiTensor(size, stride, dtype, dev, this);
...
DIOPI_RT_API diopiError_t diopiRequireTensor(diopiContextHandle_t ctx,
                                             ...
                                             const diopiDtype_t dtype,
                                             const diopiDevice_t dev) {
  diopi_log("requires a Tensor, size:[%16p, %" PRId64
            "], stride:%16p, dtype:%d[%s], device:%d[%s]",
            size->data,
            size->len,
            stride,
            dtype,
...
DIOPI_RT_API diopiError_t diopiRequireBuffer(diopiContextHandle_t ctx,
                                             ...
                                             int64_t bytes,
                                             diopiDevice_t dev) {
  diopi_log("requires a buffer, bytes: %" PRId64 ", device: %s",
            bytes,
            deviceToStr(dev));
...
DIOPI_RT_API diopiError_t diopiDestoryTensor(diopiContextHandle_t ctx,
                                             diopiTensorHandle_t tensor) {
...
DIOPI_RT_API diopiError_t diopiTensorCopyFromBuffer(
    diopiContextHandle_t ctx, const void* src, diopiTensorHandle_t tensor) {
...
DIOPI_RT_API diopiError_t diopiTensorCopyToBuffer(
    diopiContextHandle_t ctx, diopiConstTensorHandle_t tensor, void* dst) {
  if (tensor->device() == diopi_device) {
    diopiStreamHandle_t stream;
    diopiGetStream(ctx, &stream);
