...
bool isLikeChannelsLast(
  ...
  if (totalSize == 0)
    return false;
  if (stride.data[0] == stride.data[1])
    return false;
  if (checkContiguous) {
...
diopiMemoryFormat_t probableMemoryFormat(diopiConstTensorHandle_t tensor,
                                         bool exactMatch = false) {
  return isLikeChannelsLast(tensor, exactMatch)
             ? diopiMemoryFormat_t::ChannelsLast
             : (isLikeChannelsLast(tensor, exactMatch,
...
bool isContiguous(
    diopiSize_t size, diopiSize_t stride_diopi,
    diopiMemoryFormat_t format = diopiMemoryFormat_t::Contiguous) {
  auto dim = size.len;
...
setIntersection(std::vector<diopiMemoryFormat_t> matchedMemoryFormat,
  ...
  for (auto i : matchedMemoryFormat) {
    if (std::find(supportMemoryFormat.begin(), supportMemoryFormat.end(), i) !=
        std::end(supportMemoryFormat)) {
      ret.push_back(i);
    }
  }
  return ret;
