#ifndef DIOPI_ADAPTOR_CSRC_UTILS_HPP_
#define DIOPI_ADAPTOR_CSRC_UTILS_HPP_
#include <algorithm>
#include <cassert>
#include <chrono>
#include <cstdlib>
...
public:
  TimeElapsedRecord(const char *fileName) : enableTiming_(false) {
    ...
    if (enableEnvVar && strcmp(enableEnvVar, "OFF") &&
        strcmp(enableEnvVar, "0")) {
      enableTiming_ = true;
      stream_ =
          std::move(std::ofstream(fileName, std::ios::out | std::ios::trunc));
    }
  }
...
public:
  ...
  ~TimeElapsed() {
    if (timeElapsedRecord_.isEnableTiming()) {
      ...
      timeElapsedRecord_.getOStream()
          << opName_ << ": " << elapsedTime << "ms" << std::endl;
    }
  }
...
std::vector<int64_t>
calcStrides(diopiSize_t size,
            diopiMemoryFormat_t format = diopiMemoryFormat_t::Contiguous);

bool isLikeChannelsLast(
    diopiConstTensorHandle_t tensor, bool checkContiguous,
    diopiMemoryFormat_t format = diopiMemoryFormat_t::ChannelsLast);

inline diopiMemoryFormat_t probableMemoryFormat(diopiConstTensorHandle_t tensor,
                                                bool exactMatch = false);

bool isContiguous(diopiSize_t size, diopiSize_t stride_diopi,
                  diopiMemoryFormat_t format = diopiMemoryFormat_t::Contiguous);

std::vector<diopiMemoryFormat_t> matchMemoryFormatBySize(size_t sizeLen);

std::vector<diopiMemoryFormat_t>
setIntersection(std::vector<diopiMemoryFormat_t> matchedMemoryFormat,
                std::vector<diopiMemoryFormat_t> supportMemoryFormat);

template <typename T> struct RemoveConst {
  using type = T;
};

template <> struct RemoveConst<diopiConstTensorHandle_t> {
  using type = diopiTensorHandle_t;
};

std::vector<diopiMemoryFormat_t> obtainTargetMemoryFormats(
    size_t shapeLen, std::vector<diopiMemoryFormat_t> supportMemoryFormats) {
  std::vector<diopiMemoryFormat_t> matchedMemoryFormat =
      matchMemoryFormatBySize(shapeLen);
  supportMemoryFormats =
      setIntersection(matchedMemoryFormat, supportMemoryFormats);
  return supportMemoryFormats;
}

template <class T, class strategy>
ConvertType castImpl(
    diopiContextHandle_t ctx, T src, T *dst,
  ...
  std::vector<diopiMemoryFormat_t> targetMemoryFormats =
      obtainTargetMemoryFormats(srcSize.len, supportMemoryFormats);
...
template <class T, class strategy>
ConvertType requireTensorIfMemoryFormatConvert(
    diopiContextHandle_t ctx, T src, T *dst,
    std::vector<diopiMemoryFormat_t> supportMemoryFormats = defaultFormats) {
  ConvertType convertType;
  if (!src) {
...
template <class strategy> class DiopiTensorWrapper {
...
public:
  DiopiTensorWrapper(
      diopiContextHandle_t ctx, diopiTensorHandle_t payload,
    ...
    if (inp) {
      convertType_ = castImpl<diopiTensorHandle_t, strategy>(
          ctx, payload_, &tmp_, supportMemoryFormat);
    } else {
      convertType_ =
          requireTensorIfMemoryFormatConvert<diopiTensorHandle_t, strategy>(
              ctx, payload_, &tmp_, supportMemoryFormat);
    }
  }
  ...
  ~DiopiTensorWrapper() {
    ...
    if (convertType_.isMemoryFormatConverted()) {
      diopiContiguous(ctx_, &memoryFormatedTensor, tmp_,
...
#endif // DIOPI_ADAPTOR_CSRC_UTILS_HPP_