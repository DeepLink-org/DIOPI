 ...
 * @param[in] padding an array with dimension matching the number of input spatial dimensions. type = [int32, int64].
 * @param[in] dilation an array with dimension matching the number of input spatial dimensions. type = [int32, int64].
 * @param[in] transposed indicating whether the convolution is transposed.
 ...
 * @param[out] grad_input the grad of input. type = [float32, float64].
 * @param[in] grad_output the grad of output. type = [float32, float64].
 * @param[in] input_is_result boolean.
 * @sa Other parameters refer to diopiLeakyRelu().
 */
...
/**
 * @brief Applies a 2D max pooling over an input signal composed of several input planes, and record the indices of the max values.
 * @param[in] ctx Context environment.
 * @param[in] input the input tensor. type = [float16, float32]
 * @param[in] kernel_size an array, size of the pooling region. type = [int32, int64].
 * @param[in] stride an array, stride of the pooling operation. type = [int32, int64].
 ...
 * @param[in] ceil_mode boolean, if True, use ceil instead of the default floor operation when computing the output shape.
 * This ensures that every element in the input tensor is covered by a sliding window.
 * @param[out] indices the tensor contains the flattened index positions of each maximum value in the max pooling operation. type = [int32, int64].
 * @param[out] out the output tensor. type = [float16, float32].
 */
...
/**
 * @brief Applies a 2D adaptive max pooling over an input signal composed of several input planes, and record the indices of the max values.
 * @param[in] ctx Context environment.
 * @param[in] input the input tensor. type = [float32, float16, float64]
 * @param[in] output_size an array, the size of the output tensor. type = [int32, int64].
 * @param[out] out the output tensor. type = [float32, float16, float64].
...
/**
 * \brief Loss used in RetinaNet for dense detection:https://arxiv.org/abs/1708.02002.
 * @param[in] ctx Context environment.
 ...
 * @param[in] ctx Context environment.
 * @param[in] exponent the value of exponent. type = [int32, int64, uint8, int8, int16, float32, float64, float16, bool].
 * @param[in] input the input tensor and will be stored result tensor. type = [int32, int64, uint8, int8, int16, float32, float64, float16].
 */
 ...
 */
DIOPI_API diopiError_t diopiEmbeddingRenorm_(diopiContextHandle_t ctx, diopiTensorHandle_t input, diopiConstTensorHandle_t indices, double max_norm,
                                             double norm_type);
/**
 ...
 * @param[in] input the input tensor. type=[float16, float32, float64].
 * @param[in] grad the grad tensor. type=[float16, float32, float64].
 * @param[in] exp_avgs the exponentially weighted moving average of gradients. type=[float16, float32,
 * float64].
 * @param[in] exp_avg_sqs the exponentially weighted moving average of squared gradients. type=[float16, float32, float64].
 * @param[in] max_exp_avg_sqs the maximum values of the exponentially weighted moving average of squared gradients. type=[float16, float32, float64].
 ...
 */
DIOPI_API diopiError_t diopiAdam(diopiContextHandle_t ctx, diopiTensorHandle_t input, diopiTensorHandle_t grad, diopiTensorHandle_t exp_avgs,
                                 diopiTensorHandle_t exp_avg_sqs, diopiTensorHandle_t max_exp_avg_sqs, float lr, float beta1, float beta2, float eps,
                                 float weight_decay, int64_t step, bool amsgrad);
 ...
 * @param[in] momentum_buffer the buffer of momentum. type=[float16, float32, float64].
 * @param[in] lr learning rate. type=[float32].
...
DIOPI_API diopiError_t diopiRmsprop(diopiContextHandle_t ctx, diopiTensorHandle_t input, diopiTensorHandle_t grad, diopiTensorHandle_t square_avg,
                                    diopiTensorHandle_t grad_avg, diopiTensorHandle_t momentum_buffer, float lr, float alpha, float eps, float weight_decay,
 ...
 * @param[in] input the input tensor and will be stored result tensor. type = [float32, float64, float16, int16, int32, int64, uint8, int8, bool].
 ...
 * @param[in] from
 * @param[in] to
 * @param[in] inout the output tensor.
 */
 ...
 * @param[in] ctx Context environment.
 * @param[in] abs the absolute value of the complex tensor, must be float or double.
 * @param[in] angle the angle of the complex tensor. Must be same dtypes as abs.
 * @param[out] out the output tensor.
 */
 ...
 * @param[in] ctx Context environment.
 * @param[in] input the input tensor and output tensor.
 */
