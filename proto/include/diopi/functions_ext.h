 ...
 * @param[out] attention_out Tensor storing the result after applying flash attention. shape = [batch_size, q_head_num, q_seq_len, head_dim] or [batch_size,
 q_seq_len, q_head_num * head_dim]. type = [bfloat16, float16, float32].
 ...
 * @param[in] attention_mask (optional Tensor) – Attention mask. shape (batch_size, q_seq_len, kv_seq_len). Two types of masks are supported. A boolean mask
 ...
 attention score.
 * @param[in] q Query tensor. shape = [batch_size, q_head_num, q_seq_len, head_dim] or [batch_size, q_seq_len, q_head_num * head_dim]. type = [bfloat16,
 float16, float32].
 * @param[in] k Key tensor. shape = [batch_size, kv_head_num, kv_seq_len, head_dim] or [batch_size, kv_seq_len, kv_head_num * head_dim]. type = [bfloat16,
 float16, float32].
 * @param[in] v Value tensor. shape = [batch_size, kv_head_num, kv_seq_len, head_dim] or [batch_size, kv_seq_len, kv_head_num * head_dim]. type = [bfloat16,
 float16, float32].
 ...
 * @param[in] ctx The diopi context.
 * @param[out] grad_q The gradient of the query tensor. shape = [batch_size, q_head_num, q_seq_len, head_dim] or [batch_size, q_seq_len, q_head_num * head_dim].
 * type = [bfloat16, float16, float32].
 * @param[out] grad_k The gradient of the key tensor. shape = [batch_size, kv_head_num, kv_seq_len, head_dim] or [batch_size, kv_seq_len, kv_head_num *
 * head_dim]. type = [bfloat16, float16, float32].
 * @param[out] grad_v The gradient of the value tensor. shape = [batch_size, kv_seq_len,  head_dim]. type = [bfloat16, float16, float32].
 * @param[in] grad_out The gradient of the output tensor. shape = [batch_size, q_head_num, q_seq_len, head_dim] or [batch_size, q_seq_len, q_head_num *
 * head_dim].  type = [bfloat16, float16, float32].
 * @param[in] k k Key tensor. shape = [batch_size, kv_head_num, kv_seq_len, head_dim] or [batch_size, kv_seq_len, kv_head_num * head_dim]. type = [bfloat16,
 * float16, float32].
 * @param[in] v Value tensor. shape = [batch_size, kv_head_num, kv_seq_len, head_dim] or [batch_size, kv_seq_len, kv_head_num * head_dim]. type = [bfloat16,
 * float16, float32].
 * @param[in] attention_out Tensor storing the result after applying flash attention. shape = [batch_size, q_head_num, q_seq_len, head_dim] or [batch_size,
 * q_seq_len, q_head_num * head_dim]. type = [bfloat16, float16, float32].
