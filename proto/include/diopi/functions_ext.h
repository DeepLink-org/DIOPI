 ...
 * @param[in] attention_mask (optional Tensor) – Attention mask. shape (batch_size, q_seq_len, kv_seq_len). Two types of masks are supported. A boolean mask
 ...
 attention score.
 * @param[in] q Query tensor. shape = [batch_size, q_head_num, q_seq_len, head_dim] or [batch_size, q_seq_len, embedding_dim]. type = [bfloat16,
 float16, float32].
 * @param[in] k Key tensor. shape = [batch_size, kv_head_num, kv_seq_len, head_dim] or [batch_size, kv_seq_len, embedding_dim]. type = [bfloat16,
 float16, float32].
 * @param[in] v Value tensor. shape = [batch_size, kv_head_num, kv_seq_len, head_dim] or [batch_size, kv_seq_len, embedding_dim]. type = [bfloat16,
 float16, float32].
 ...
 * @param[in] q Query tensor. shape = [batch_size, q_head_num, q_seq_len, head_dim] or [batch_size, q_seq_len, embedding_dim]. type = [bfloat16,
 float16, float32].
 * @param[in] k k Key tensor. shape = [batch_size, kv_head_num, kv_seq_len, head_dim] or [batch_size, kv_seq_len, embedding_dim]. type = [bfloat16,
 * float16, float32].
 * @param[in] v Value tensor. shape = [batch_size, kv_head_num, kv_seq_len, head_dim] or [batch_size, kv_seq_len, embedding_dim]. type = [bfloat16,
 * float16, float32].
...
DIOPI_API diopiError_t diopiAttentionBackward(diopiContextHandle_t ctx, diopiTensorHandle_t grad_q, diopiTensorHandle_t grad_k, diopiTensorHandle_t grad_v,
                                              diopiConstTensorHandle_t grad_out, diopiConstTensorHandle_t q, diopiConstTensorHandle_t k,
                                              diopiConstTensorHandle_t v, diopiConstTensorHandle_t attention_out, diopiConstTensorHandle_t* saved_for_backward,
