...
int aclnnSinAdaptor(diopiContextHandle_t ctx, diopiConstTensorHandle_t self1, diopiTensorHandle_t out1) {
    AscendTensor at(out1);
    std::cout << "sin tensor ptr=" << at.data() << std::endl;
    aclrtStream stream;
    diopiGetStream(ctx, &stream);
    // 1.构造输入与输出，需要根据API的接口自定义构造
    aclTensor* self = nullptr;
    ...
    std::cout << "aclnnSinGetWorkspaceSize ptr = " << reinterpret_cast<void*>(&aclnnSinGetWorkspaceSize) << std::endl;
...
int aclnnCosAdaptor(diopiContextHandle_t ctx, diopiConstTensorHandle_t self1, diopiTensorHandle_t out1) {
    AscendTensor at(out1);
    std::cout << "cos tensor ptr=" << at.data() << std::endl;
    aclrtStream stream;
    diopiGetStream(ctx, &stream);
    // 1.构造输入与输出，需要根据API的接口自定义构造
    aclTensor* self = nullptr;
