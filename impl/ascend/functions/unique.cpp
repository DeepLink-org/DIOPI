...
diopiError_t diopiUnique1(diopiContextHandle_t ctx, diopiTensorHandle_t* out, diopiConstTensorHandle_t input, const int64_t* dim, bool sorted,
    ...
    AscendTensor inputTmpAt(input), inputAt;
    contiguous(ctx, inputTmpAt, inputAt);
    ...
    // if (return_inverse) {
        if (dim) {
            ...
            std::vector<int64_t> inSizeVec = {inputAt.shape(dim_value)};
            diopiSize_t inSize = {inSizeVec.data(), static_cast<int64_t>(inSizeVec.size())};
    ...
    std::cout << "finish aclnnUniqueConsecutive." << std::endl;
    ...
    std::cout << "finish outAt aclnnInplaceCopy." << std::endl;
    ...
    if (return_inverse) {
        ...
        // inverseTmpAt.view(indicesAt.shape());
        std::cout << dumpTensor(inverseTmpAt, "inverseTmpAt") << std::endl;
        std::cout << dumpTensor(indicesAt, "indicesAt") << std::endl;
        DIOPI_ASCEND_CALL_ACLNN(aclnnInplaceCopy, ctx, indicesAt, inverseTmpAt);
        std::cout << "finish indicesAt aclnnInplaceCopy." << std::endl;
    }
    ...
    if (return_counts) {
        ...
        std::vector<int64_t> tmp{countShape.data, countShape.data + countShape.len};        // shape 是错误的。
        std::cout << "count value=" << std::endl;
        for (auto i : tmp) {
            std::cout << i << std::endl;
        }
        std::cout << "finish count value=" << std::endl;
        std::cout << "tuple_size=" << std::tuple_size<decltype(params)>::value << std::endl;
        std::cout << dumpTensor(countsTmpAt, "countsTmpAt before resize.") << std::endl;
        ...
        std::cout << dumpTensor(countsTmpAt, "countsTmpAt") << std::endl;
        std::cout << dumpTensor(countsAt, "countsAt") << std::endl;
        printContiguousTensor(ctx, countsAt, "countsAt alllllll");
        DIOPI_ASCEND_CALL_ACLNN(aclnnInplaceCopy, ctx, countsAt, countsTmpAt);
        std::cout << "finish countsAt aclnnInplaceCopy." << std::endl;
    }
