...
diopiError_t diopiAdd(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t input, diopiConstTensorHandle_t other,
                      const diopiScalar_t* alphaPtr) {
#if 1
    if (isScalarOne(alphaPtr)) {
        AclOpRunner<2, 1, dtypeConvertor>("Add", ctx).addInput(input).addInput(other).addOutput(out).run();
    } else {
        diopiScalar_t alphaTmp;
        if (alphaPtr->stype == diopi_dtype_float64) {
            double alphaVal = alphaPtr->fval;
            alphaTmp = constructDiopiScalarT(diopi_dtype_float32, alphaVal);
        }

        if (alphaPtr->stype == diopi_dtype_int64) {
            double alphaVal = alphaPtr->ival;
            alphaTmp = constructDiopiScalarT(diopi_dtype_int32, alphaVal);
        }

        AclOpRunner<3, 1>("AxpyV2", ctx).addInput(input).addInput(other).addConstInput(alphaTmp).addOutput(out).run();
    }

#else
    auto ret = aclnnAddAdaptor(ctx, input, other, alphaPtr, out);
#endif
    return diopiSuccess;
}

diopiError_t diopiAddInp(diopiContextHandle_t ctx, diopiTensorHandle_t input, diopiConstTensorHandle_t other, const diopiScalar_t* alphaPtr) {
    return diopiAdd(ctx, input, input, other, alphaPtr);
}
...
diopiError_t diopiAddScalar(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t input, const diopiScalar_t* other,
    ...
    return diopiAdd(ctx, out, input, trOther, alphaPtr);
}

diopiError_t diopiAddInpScalar(diopiContextHandle_t ctx, diopiTensorHandle_t input, const diopiScalar_t* other, const diopiScalar_t* alphaPtr) {
    return diopiAddScalar(ctx, input, input, other, alphaPtr);
}
...
diopiError_t diopiSub(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t input, diopiConstTensorHandle_t other,
    ...
    const float value = (alphaPtr != nullptr) ? getValue<float>(alphaPtr) : 1.0;
    if (value == 1.0) {
        ...
        AclOpRunner<2, 1>("Axpy", ctx).addInput(input, highType).addInput(other, highType).setAttr<float>("alphaPtr", -value).addOutput(outCopy).run();
    }
    if (outDtype != highType) diopiCastDtype(ctx, out, outCopy);
    return diopiSuccess;
}

diopiError_t diopiSubInp(diopiContextHandle_t ctx, diopiTensorHandle_t input, diopiConstTensorHandle_t other, const diopiScalar_t* alphaPtr) {
    return diopiSub(ctx, input, input, other, alphaPtr);
}
...
diopiError_t diopiSubScalar(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t input, const diopiScalar_t* other,
    ...
    diopiGetTensorDtype(out, &dtype);
    makeTensorFromScalar(ctx, other, &trOther, dtype, diopiDevice_t::diopi_device);
    return diopiSub(ctx, out, input, trOther, alphaPtr);
}

diopiError_t diopiSubInpScalar(diopiContextHandle_t ctx, diopiTensorHandle_t input, const diopiScalar_t* other, const diopiScalar_t* alphaPtr) {
    return diopiSubScalar(ctx, input, input, other, alphaPtr);
}
