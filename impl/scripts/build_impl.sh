...
case $1 in
  ...
  ascend_npu)
    mkdir -p build && cd build && cmake ..  -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DIMPL_OPT=ascend_npu -DTEST=ON && make -j;;
  ascend)
    mkdir -p build && cd build && cmake ..  -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DIMPL_OPT=ascend -DTEST=ON && make -j;;
  hip_pytorch)
