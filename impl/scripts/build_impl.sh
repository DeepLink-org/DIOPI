...
case $1 in
  ...
  droplet)
    mkdir build && cd build && cmake .. -DIMPL_OPT=droplet  -DTEST=ON -DCMAKE_BUILD_TYPE=Release && make -j8 ;;
  supa)
    mkdir -p build && cd build && cmake .. -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DIMPL_OPT=supa -DCMAKE_BUILD_TYPE=Release -DTEST=ON && make -j8 ;;
  *)
