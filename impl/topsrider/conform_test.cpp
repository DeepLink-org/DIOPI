...
#define CALL_TOPS(Expr)                                                      \
  {                                                                          \
    ...
    if (ret != topsSuccess) {                                                \
      printf(                                                                \
          "call a topsrt function (%s) failed. return code=%d", #Expr, ret); \
    }                                                                        \
  }
...
int32_t device_memcpy_h2d_async(diopiStreamHandle_t stream_handle,
  ...
  auto* ph_stream = (topsStream_t)stream_handle;
  CALL_TOPS(
      topsMemcpyAsync(dst, src, bytes, topsMemcpyHostToDevice, ph_stream));
  return diopiSuccess;
}
...
int32_t device_memcpy_d2h_async(diopiStreamHandle_t stream_handle,
  ...
  auto* ph_stream = (topsStream_t)stream_handle;
  CALL_TOPS(
      topsMemcpyAsync(dst, src, bytes, topsMemcpyDeviceToHost, ph_stream));
  return diopiSuccess;
}
...
int32_t device_memcpy_d2d_async(diopiStreamHandle_t stream_handle,
  ...
  auto* ph_stream = (topsStream_t)stream_handle;
  CALL_TOPS(
      topsMemcpyAsync(dst, src, bytes, topsMemcpyDeviceToDevice, ph_stream));
  return diopiSuccess;
}
