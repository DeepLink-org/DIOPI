...
__device__ __forceinline__ float sum_reduce(float acc, float x) { return acc + x; }
...
__device__ __forceinline__ index_t binary_search_segment_number(const index_t* seg_offsets, const index_t n_seg, const index_t n_elem, const index_t elem_id) {
