...
#include <ATen/cuda/ApplyGridUtils.cuh>  // IWYU pragma: export
#include <THC/THCAtomics.cuh>            // IWYU pragma: export
#include <algorithm>
#include <type_traits>
...
constexpr __host__ __device__ inline IntType ceil_div(IntType n, IntType m) {
    static_assert(std::is_integral<IntType>::value, "ceil_div only accept integral types");
    return (n + m - 1) / m;
}
