...
diopiError_t diopiConvolution2dBackward(diopiContextHandle_t ctx, diopiTensorHandle_t grad_input, diopiTensorHandle_t grad_weight,
...
#else
    {
        ...
        if (grad_input || grad_weight) {
            ...
            auto grad_inputs = at::convolution_backward(atGrad,
                                                        atInput,
                                                        atWeight,
                                                        c10::nullopt,
                                                        atStride,
                                                        ...
                                                        atOutputPadding,
                                                        groups,
                                                        {grad_input != nullptr, grad_weight != nullptr, false});
            if (grad_input) {
        ...
        if (grad_bias != nullptr) {
            auto atGradBias = impl::aten::buildATen(grad_bias);
            const int ndim = atGrad.dim();
            std::vector<int64_t> reduceDims(ndim - 1);
            if (ndim == 3) {
                reduceDims = {1, 2};
            } else {
                reduceDims = {0, 2, 3};
            }
            at::sum_out(atGradBias, atGrad, reduceDims, false);
        }
    }
...
diopiError_t diopiLinearBackward(diopiContextHandle_t ctx, diopiTensorHandle_t grad_input, diopiTensorHandle_t grad_weight, diopiTensorHandle_t grad_bias,
    ...
    if (grad_weight) {
        ...
        if (dims > 2) {
            std::vector<int64_t> sumDim(dims - 2);
