...
namespace {
diopiError_t convolution_forward(diopiContextHandle_t ctx, DiopiTensor input, DiopiTensor weight, DiopiTensor bias, DiopiTensor output, diopiSize_t stride,
...
diopiError_t convolution_backward_input(diopiContextHandle_t ctx, DiopiTensor gradOutput, DiopiTensor gradInput, DiopiTensor weight, diopiSize_t stride,
                                        diopiSize_t padding, diopiSize_t dilation, int64_t groups) {
    cnnlHandle_t handle = cnnlHandlePool.get(ctx);
...
diopiError_t convolution_backward_weight(diopiContextHandle_t ctx, DiopiTensor gradOutput, DiopiTensor gradWeight, DiopiTensor input, diopiSize_t stride,
                                         diopiSize_t padding, diopiSize_t dilation, int64_t groups) {
    cnnlHandle_t handle = cnnlHandlePool.get(ctx);
    CnnlTensorDesc gradOutputDesc(gradOutput, CNNL_LAYOUT_NHWC, gradOutput.shape<int32_t>());
...
// diopiError_t convolution_forward_bias(diopiContextHandle_t ctx, DiopiTensor Output, DiopiTensor bias) {
//     cnnlHandle_t handle = cnnlHandlePool.get(ctx);
//     CnnlTensorDesc gradOutputDesc(gradOutput, CNNL_LAYOUT_NHWC, gradOutput.shape<int32_t>());
...
diopiError_t convolution_backward_bias(diopiContextHandle_t ctx, DiopiTensor gradOutput, DiopiTensor gradBias) {
    cnnlHandle_t handle = cnnlHandlePool.get(ctx);
    CnnlTensorDesc gradOutputDesc(gradOutput, CNNL_LAYOUT_NHWC, gradOutput.shape<int32_t>());
    CnnlTensorDesc biasGradDesc(gradBias, CNNL_LAYOUT_ARRAY);
    std::vector<int64_t> biasShape = gradBias.shape();
...
extern "C" diopiError_t diopiConvolution2d(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t input, diopiConstTensorHandle_t weight,
    ...
    DIOPI_CALL(convolution_forward(ctx, inputTensorTr, weightTensorTr, biasTensorCasted, outputTensorTr, stride, padding, dilation, groups));
...
extern "C" diopiError_t diopiConvolution2dBackward(diopiContextHandle_t ctx, diopiTensorHandle_t gradInput, diopiTensorHandle_t gradWeight,
    ...
    DIOPI_CALL(convolution_backward_input(ctx, gradOutputTr, gradInputTr, weightTr, stride, padding, dilation, groups));
    DIOPI_CALL(convolution_backward_weight(ctx, gradOutputTr, gradWeightTr, inputTr, stride, padding, dilation, groups));
    ...
    if (biasGradTensor.defined()) {
        ...
        DIOPI_CALL(autoCastTensorType(ctx, tensors, {diopi_dtype_float16, diopi_dtype_float32}));
        DIOPI_CALL(convolution_backward_bias(ctx, gradOutputTr, gradBiasCasted));
...
extern "C" diopiError_t diopiConvTranspose2d(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t input, diopiConstTensorHandle_t weight,
    ...
    DIOPI_CALL(convolution_backward_input(ctx, inputTr, outputTr, weightTr, stride, padding, dilation, groups));
...
extern "C" diopiError_t diopiConvTranspose2dBackward(diopiContextHandle_t ctx, diopiTensorHandle_t gradInput, diopiTensorHandle_t gradWeight,
    ...
    DIOPI_CALL(convolution_forward(ctx, gradOutputTr, weightTr, {}, gradInputTr, stride, padding, dilation, groups));
    DIOPI_CALL(convolution_backward_weight(ctx, inputTr, gradWeightTr, gradOutputTr, stride, padding, dilation, groups));
