...
diopiError_t makeTensorFromScalar(diopiContextHandle_t ctx, const diopiScalar_t* scalar, DiopiTensor& out) {
    ...
    if (DiopiDataType::isIntegerWithBool(scalar->stype)) {
        ...
        return diopiSuccess;
    } else if (DiopiDataType::isFloatPoint(scalar->stype)) {
        ...
        setLastErrorString("%s", "salar dtype is not bool, int or float");
