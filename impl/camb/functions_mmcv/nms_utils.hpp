...
#ifndef IMPL_CAMB_FUNCTIONS_MMCV_NMS_UTILS_HPP_
#define IMPL_CAMB_FUNCTIONS_MMCV_NMS_UTILS_HPP_
...
#define REDUCE_NUM (7)  // score, x1, y1, x2, y2, max_index (reserve 2 num for half-type input)
...
template <typename T>
static __mlu_func__ void computeReluN(T *nram_dst, T *nram_src, void *nram_tmp, const int deal_num, const T threshold = 0) {
    if (threshold < 0) {
        return;
    }
    if (threshold) {
...
__mlu_func__ void getComputeParamsBlockOrU1(const int input_dwidth, const int input_box_num, const int limit, const int core_limit, int &input_offset,
                                            int &max_seg_pad, int &repeat, int &remain, int &remain_pad, int &max_seg_iou_compute, int &repeat_iou_compute,
...
__mlu_func__ void getComputeParamsUx(const int input_dwidth, const int input_num_boxes, const int limit, int &input_offset, int &max_seg_pad, int &repeat,
                                     int &remain, int &remain_pad, int &max_seg_iou_compute, int &repeat_iou_compute, int &remain_iou_compute,
    ...
    int cluster_offset = avg_cluster * clusterId + (clusterId <= rem_cluster ? clusterId : rem_cluster);
    ...
    remain_pad = NMS_UP(remain, NMS_SIZE);
    // if datatype is fp16, we should cvt to fp32 when compute iou
    max_seg_iou_compute = NMS_DOWN(max_seg_pad / (sizeof(float) / input_dwidth), NMS_SIZE);
    repeat_iou_compute = len_core / max_seg_iou_compute;
    remain_iou_compute = len_core % max_seg_iou_compute;
    remain_pad_iou_compute = NMS_UP(remain_iou_compute, NMS_SIZE);
}
...
__mlu_func__ void findGlobalMaxBox(IN_DT *max_box, IN_DT *sram, IN_DT *inter_x1) {
    // copy all partial max to the sram of cluster 0
    if (clusterId != 0) {
        __memcpy(sram + REDUCE_NUM * clusterId, sram, REDUCE_NUM * sizeof(IN_DT), SRAM2SRAM, 0);
    }
    __sync_all();
    ...
    if (clusterId == 0) {
        ...
        if (coreId == 0x80 && clusterDim > 1) {
            ...
            for (int cluster_idx = 1; cluster_idx < clusterDim; ++cluster_idx) {
                __memcpy(sram, sram, REDUCE_NUM * sizeof(IN_DT), SRAM2SRAM, cluster_idx);
            }
        }
        __sync_cluster();
    }
...
template <typename IN_DT>
__mlu_func__ void findCoreMaxBox(IN_DT *input_score_ptr, IN_DT *score, IN_DT *inter_x1, IN_DT *max_box, const IN_DT *input_x1_ptr, const IN_DT *input_y1_ptr,
                                 const IN_DT *input_x2_ptr, const IN_DT *input_y2_ptr, const mluMemcpyDirection_t load_dir, const int input_offset,
                                 const int repeat, const int remain, const int remain_pad, const int max_seg_pad, int &max_index) {
    if (coreId != 0x80) {
        for (int i = 0; i <= repeat; i++) {
            ...
            __memcpy(score,
                     input_score_ptr + input_offset + i * max_seg_pad,
...
template <typename IN_DT>
__mlu_func__ void findClusterMaxBox(IN_DT *sram, IN_DT *max_box, IN_DT *inter_x1, IN_DT *input_data_score, const int core_limit) {
...
template <typename IN_DT>
__mlu_func__ void calMaxArea(IN_DT *max_box, const int algo, float offset, float &max_area, float &max_box_x1, float &max_box_y1, float &max_box_x2,
                             float &max_box_y2) {
    ...
    if (algo == 0 || offset == 0.0) {
        ...
        max_area = (max_box_x2 - max_box_x1 + offset) * (max_box_y2 - max_box_y1 + offset);
    }
}
...
__mlu_func__ void storeResult(IN_DT *max_box, OUT_DT *nram_save, OUT_DT *&output_dram, const int keep, const int nram_save_limit_count,
    ...
    if (static_cast<float>(max_box[0]) > thresh_score) {
        ...
        if (clusterId == 0 && coreId == 0) {
            if (output_mode == 0) {  // index1, index2, ...
                ...
                __memcpy(save_ptr + save_offset, max_box, 1 * sizeof(IN_DT), NRAM2NRAM, save_str_num * sizeof(IN_DT), 1 * sizeof(IN_DT), 4);
            }
        }
        nram_save_count++;
        output_box_num++;
        ...
        if ((nram_save_count == nram_save_limit_count) || (static_cast<float>(max_box[0]) <= thresh_score) || keep == max_output_size - 1) {
            if (nram_save_count != 0) {
                if (clusterId == 0 && coreId == 0) {
                    if (output_mode == 0) {  // index1, index2, ...
                        pvLock();
                        __memcpy(output_dram, nram_save, nram_save_count * sizeof(uint32_t), NRAM2GDRAM);
                        pvUnlock();
                        output_dram += nram_save_count;
                    } else if (output_mode == 1) {  // score, x1, y1, x2, y2
                        pvLock();
                        __memcpy(output_dram, nram_save, nram_save_count * INFO_NUM * sizeof(IN_DT), NRAM2GDRAM);
                        pvUnlock();
                        output_dram += nram_save_count * INFO_NUM;
                    } else if (output_mode == 2) {  // score---, x1---, y1---, x2---, y2---
                        pvLock();
                        __memcpy(output_dram,
                                 nram_save,
                                 nram_save_count * sizeof(IN_DT),
                                 NRAM2GDRAM,
                                 max_output_size * sizeof(IN_DT),
                                 nram_save_limit_count * sizeof(IN_DT),
                                 4);
                        pvUnlock();
                        output_dram += nram_save_count;
                    }
...
template <typename IN_DT, typename OUT_DT>
__mlu_func__ void scoreUpdate(IN_DT *input_score_ptr, const mluMemcpyDirection_t load_dir, const mluMemcpyDirection_t store_dir, const IN_DT *input_x1_ptr,
                              const IN_DT *input_y1_ptr, const IN_DT *input_x2_ptr, const IN_DT *input_y2_ptr, IN_DT *x1, IN_DT *y1, IN_DT *x2, IN_DT *y2,
                              IN_DT *score, IN_DT *inter_x1, IN_DT *inter_y1, IN_DT *inter_x2, IN_DT *inter_y2, IN_DT *max_box, const float max_box_x1,
                              const float max_box_y1, const float max_box_x2, const float max_box_y2, OUT_DT *nram_save, int repeat_iou_compute,
                              int remain_iou_compute, int remain_pad_iou_compute, int max_seg_iou_compute, int max_seg_pad, const float thresh_iou,
                              const float div_thresh_iou, const int input_offset, const float offset, const float max_area, const int input_num_boxes,
                              const int algo) {
    for (int i = 0; i <= repeat_iou_compute; i++) {
        if (i == repeat_iou_compute && remain_iou_compute == 0) {
            break;
        }
        int seg_len = (i == repeat_iou_compute) ? remain_pad_iou_compute : max_seg_iou_compute;
        int cpy_len = (i == repeat_iou_compute) ? remain_iou_compute : max_seg_iou_compute;
        /******NMS LOAD START******/
        int dt_offset = 0;
        if (sizeof(IN_DT) == sizeof(float)) {
            ...
            __memcpy(x1,
                     input_score_ptr + input_offset + i * max_seg_iou_compute,
                     cpy_len * sizeof(IN_DT),
                     load_dir,
                     cpy_len * sizeof(IN_DT),
                     cpy_len * sizeof(IN_DT),
...
#endif  // IMPL_CAMB_FUNCTIONS_MMCV_NMS_UTILS_HPP_
