...
diopiError_t diopiTokenSoftmaxReduceVInference(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t logics, diopiConstTensorHandle_t v,
    ...
    for (int i = 0; i < batch; ++i) {
        ...
        at::Tensor p = op_api::index(logicsAt, {at::Tensor(), op_api::arange(curSeqStartLoc, curSeqStartLoc + curSeqLen, at::kInt, layout, device)})
                           .softmax(-1)
                           .reshape({head, 1, 1, curSeqLen})
                           .transpose(0, 1);
        at::Tensor vLoc = bLocAt[i].index_select(0, op_api::arange(maxInputLen - curSeqLen, maxInputLen, at::kInt, layout, device));
        at::Tensor v = op_api::index(vAt, {vLoc}).view({1, curSeqLen, head, dim}).transpose(1, 2);
        at::Tensor values = op_api::matmul(p.toType(at::kFloat), v.toType(at::kFloat)).view({head, dim}).toType(dtype);
        op_api::index_put_(outAt, {torch::scalar_to_tensor(i)}, values);
    }
    END_CALL_ACL_OP();
}
