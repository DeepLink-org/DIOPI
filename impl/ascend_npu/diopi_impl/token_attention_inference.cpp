...
diopiError_t diopiTokenAttentionInference(diopiContextHandle_t ctx, diopiTensorHandle_t attentionOut, diopiConstTensorHandle_t q, diopiConstTensorHandle_t k,
    ...
    std::cout << "原始值: " << atAttentionOut << std::endl;
...
diopiError_t diopiTokenSoftmaxReduceVInference(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t logics, diopiConstTensorHandle_t v,
    ...
    for (int i = 0; i < batch; ++i) {
        int curSeqLen = atBSeqLen[i].item<int>();
        int curSeqStartLoc = atBStartLoc[i].item<int>();
        at::Tensor P = atLogics.slice(1, curSeqStartLoc, curSeqStartLoc + curSeqLen).softmax(-1).reshape({head, 1, 1, curSeqLen}).transpose(0, 1).to(atLogics.device());
        at::Tensor vLoc = atBLoc[i].index_select(0, at::arange(maxInputLen - curSeqLen, maxInputLen).to(atLogics.device()));
