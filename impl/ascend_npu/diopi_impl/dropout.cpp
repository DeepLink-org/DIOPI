...
static diopiError_t ascendNpuDropout2dOut(at::Tensor& out, at::Tensor& mask, const at::Tensor& input, double p, at::Generator gen) {
    ...
    op_api::mul_out(input, out2d, out);

    at::Scalar ref(0.0);
    at_npu::native::OpPreparation::check_tensor({out2d}, mask, mask.scalar_type(), out2d.sizes());
    EXEC_NPU_CMD(aclnnNeScalar, out2d, ref, mask);
