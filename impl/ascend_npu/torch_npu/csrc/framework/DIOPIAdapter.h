...
struct NPUGeneratorImpl : public c10::GeneratorImpl {
    ...
    void set_current_seed(uint64_t seed){INTERFACE_NOT_IMPL} uint64_t current_seed() const {INTERFACE_NOT_IMPL} uint64_t seed() { INTERFACE_NOT_IMPL }
    void set_state(const c10::TensorImpl &new_state){INTERFACE_NOT_IMPL} c10::intrusive_ptr<c10::TensorImpl> get_state() const { INTERFACE_NOT_IMPL }
    void set_philox_offset_per_thread(uint64_t offset){INTERFACE_NOT_IMPL} uint64_t philox_offset_per_thread() const { INTERFACE_NOT_IMPL }
    void capture_prologue(int64_t *offset_extragraph){INTERFACE_NOT_IMPL} uint64_t capture_epilogue(){INTERFACE_NOT_IMPL} PhiloxNpuState
        philox_npu_state(uint64_t increment){INTERFACE_NOT_IMPL}
