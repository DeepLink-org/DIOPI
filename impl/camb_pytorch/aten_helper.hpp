...
#define ATEN_NOT_IMPLEMENT() set_last_error_string("NotImplementError: function %s is not implemented for torch version %d" __FUNCTION__, TORCH_VERSION);
...
void MluTypeDefault::init() { return; }
...
inline void resetCurCtx(diopiContextHandle_t ctx) { context = nullptr; }
...
diopiError_t nll_loss_internal(diopiContextHandle_t ctx, diopiTensorHandle_t out, const at::Tensor& atInput, diopiConstTensorHandle_t target,
...
diopiError_t nll_loss_bp_internal(diopiContextHandle_t ctx, diopiTensorHandle_t grad_input, diopiConstTensorHandle_t grad_output, at::Tensor& atInput,
    ...
    assert(dim > 1);
    if (dim == 2) {
        auto atGradInput = torch_mlu::cnnl::ops::cnnl_nll_loss_backward(atGradOutput, atInput, atTarget, atWeight, reduction, ignore_index, atTotalWeight);
        camb::aten::updateATen2Tensor(ctx, atGradInput, grad_input);
    } else if (dim == 4) {
        auto atGradInput = torch_mlu::cnnl::ops::cnnl_nll_loss2d_backward(atGradOutput, atInput, atTarget, atWeight, reduction, ignore_index, atTotalWeight);
        camb::aten::updateATen2Tensor(ctx, atGradInput, grad_input);
    } else {
        auto n = atInput.size(0);
        auto c = atInput.size(1);
