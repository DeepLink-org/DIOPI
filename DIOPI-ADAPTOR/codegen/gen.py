...
def get_functions_support(source_dir):
    ...
    for idx, row in enumerate(content):
        if row.startswith(' * type'):
            ...
            if is_in:
                ins_from_comment.append(param_name)
            ...
            for tensor in outs:
                if tensor in ins_from_comment:
                    insert('ins_outs', func_dtypes, tensor, param_dtypes)
                else:
    ...
    for func in funcs_info:
        if 'sa' in funcs_info[func].keys():
            ...
            for i in from_func_info['ins']:
                ...
                elif i in funcs_info[func]['ins_outs']:
                    funcs_info[func]['ins_outs'][i] = from_func_info['ins'][i]
            ...
            for out in from_func_info['outs']:
                ...
                elif out in funcs_info[func]['ins_outs']:
                    funcs_info[func]['ins_outs'][out] = from_func_info['outs'][out]
                if 'grad_' + out in funcs_info[func]['ins']:
                    funcs_info[func]['ins']['grad_' + out] = from_func_info['outs'][out]
            for io in from_func_info['ins_outs']:
                if io in funcs_info[func]['ins']:
                    funcs_info[func]['ins'][io] = from_func_info['ins_outs'][io]
                elif io in funcs_info[func]['outs']:
                    funcs_info[func]['outs'][io] = from_func_info['ins_outs'][io]
                elif io in funcs_info[func]['ins_outs']:
                    funcs_info[func]['ins_outs'][io] = from_func_info['ins_outs'][io]
                if 'grad_' + io in funcs_info[func]['ins']:
                    funcs_info[func]['ins']['grad_' + io] = from_func_info['ins_outs'][io]
    return funcs_info
        ...
        for info in config:
            ...
            if op_name == 'common_config':
                ...
                if 'tensor_dtype' in op_cfg.keys():
                    for tensor in op_cfg['tensor_dtype']:
                        assert tensor in funcs_info[op_name]['ins'].keys() or  tensor in funcs_info[op_name]['outs'].keys() \
                            or tensor in funcs_info[op_name]['ins_outs'].keys()
                ...
                for tensor in list(funcs_info[op_name]['ins'].keys())+list(funcs_info[op_name]['outs'].keys()) \
                    + list(funcs_info[op_name]['ins_outs'].keys()):
    ...
    for func in func_infos:
        ...
        if (func not in op_configs.keys() and 'Common' not in op_configs.keys()) or len(list(func_infos[func].keys())) == 1:
            ...
            for tensor in list(func_infos[func]['ins'].keys()) + list(func_infos[func]['outs'].keys()) \
                + list(func_infos[func]['ins_outs'].keys()):
...
""")
                ...
                ios = func_infos[func]['ins_outs']
                if tensor in list(outs.keys()) + list(ios.keys()):
                    if ('Inp' in op_name and tensor == 'input') or tensor in ios.keys():
                        is_inplace = 'true'
                    else:
                        is_inplace = 'false'
            ...
            for arg in func_infos[func]['call_args']:
                ...
                elif name in outs.keys() or name in ios.keys():
