...
case $1 in
  clean)
    rm -rf build;;
  cuda)
    mkdir -p build && cd build && cmake .. -DIMPL_OPT=cuda -DTEST=${DIOPI_BUILD_TESTRT} && make -j8;;
  torch)
  ...
  camb_pytorch)
    ...
    && make -j8;;
  camb)
    mkdir -p build && cd build && cmake .. -DIMPL_OPT=camb -DTEST=${DIOPI_BUILD_TESTRT} -DCMAKE_EXPORT_COMPILE_COMMANDS=ON && make -j8;;
  ascend)
    mkdir -p build && cd build && cmake .. -DIMPL_OPT=ascend -DTEST=${DIOPI_BUILD_TESTRT} && make -j8;;
  hip_pytorch)
    mkdir build && cd build && cmake .. -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DIMPL_OPT=TORCH -DHIP=ON && make -j8 \
    || exit -1;;
  mmcv_ext)
