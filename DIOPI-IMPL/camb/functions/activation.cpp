...
diopiSigmoidBackward(diopiContextHandle_t ctx, diopiTensorHandle_t grad_input,
  ...
  CnnlAttribute attr;
  attr.set("mode", CNNL_ACTIVATION_SIGMOID);
  DIOPI_CALL(cnnl_activation_backward_internal(
      ctx, grad_input_tensor, grad_output_tensor, {}, output_tensor, attr));
  return diopiSuccess;
}
...
extern "C" diopiError_t diopiGeluBackward(diopiContextHandle_t ctx,
  ...
  DIOPI_CALL(cnnl_activation_backward_internal(
      ctx, grad_input_tensor, grad_output_tensor, input_tensor, {}, attr));
  return diopiSuccess;
}
...
extern "C" diopiError_t diopiLeakyRelu(diopiContextHandle_t ctx,
  ...
  CnnlAttribute attr;
  float coef_val = DiopiDataType::isInteger(negative_slope->stype)
                       ? negative_slope->ival
                       : negative_slope->fval;
...
extern "C" diopiError_t diopiLeakyReluBackward(
    ...
    const diopiScalar_t *negative_slope, bool input_is_result) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
  DiopiTensor grad_input_tensor(grad_input);
  DiopiTensor grad_output_tensor(grad_output);
  DiopiTensor input_tensor(input);
