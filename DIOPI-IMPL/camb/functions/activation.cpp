...
public:
  ...
  template <typename T> T get(std::string key, T defaultValue) const {
    auto iter = m_data.find(key);
    if (iter != m_data.end()) {
      const ValueHolderBase *holder = iter->second.get();
      if (holder->getTypeInfo() == typeid(T)) {
        const ValueHolder<T> *typedHolder =
            static_cast<const ValueHolder<T> *>(holder);
...
private:
  ...
  template <typename T> class ValueHolder : public ValueHolderBase {
  public:
...
diopiError_t cnnl_activation_internal(diopiContextHandle_t ctx,
                                      DiopiTensor input, DiopiTensor out,
                                      CnnlAttribute attr) {
  ...
  CnnlResourceGuard<cnnlActivationDescriptor_t, cnnlCreateActivationDescriptor,
                    cnnlDestroyActivationDescriptor>
      activation_desc;
  DIOPI_CALLCNNL(cnnlSetActivationDescriptor_v6(
      activation_desc.get(), mode, perf, nan_prop, coef, sliced_dim, gamma,
      scale, is_result, approximate));

  std::vector<DiopiTensor *> inputs{&input};
  DIOPI_CALL(autoCastTensorType(ctx, inputs,
  ...
  DIOPI_CALLCNNL(cnnlActivationForward(handle, activation_desc.get(), alpha,
                                       input_desc.get(), input.data(), beta,
                                       output_desc.get(), temp_output.data()));
  DIOPI_CALL(dataTypeCast(ctx, out, temp_output));

  return diopiSuccess;
}

diopiError_t cnnl_activation_backward_internal(
    diopiContextHandle_t ctx, DiopiTensor grad_input, DiopiTensor grad_output,
    DiopiTensor input, DiopiTensor output, CnnlAttribute attr) {
  auto handle = cnnlHandlePool.get(ctx);
  auto mode = attr.get<cnnlActivationMode_t>("mode", CNNL_ACTIVATION_SIGMOID);
  auto perf = attr.get<cnnlActivationPreference_t>(
      "perf", CNNL_ACTIVATION_HIGH_PRECISION);
  ...
  std::set<diopiDtype_t> support_dtype{diopi_dtype_float16,
                                       diopi_dtype_float32};
  ...
  DIOPI_CALLCNNL(cnnlActivationBackward(
      handle, activation_desc.get(), alpha, output_desc.get(),
      output.defined() ? output.data() : nullptr, grad_output_desc.get(),
      grad_output.data(), input_desc.get(),
      input.defined() ? input.data() : nullptr, beta, grad_input_desc.get(),
...
extern "C" DIOPI_API diopiError_t diopiReluInp(diopiContextHandle_t ctx,
...
extern "C" diopiError_t diopiSigmoid(diopiContextHandle_t ctx,
                                     diopiTensorHandle_t out,
                                     diopiConstTensorHandle_t input) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
  DiopiTensor input_tensor(input);
...
extern "C" diopiError_t diopiSigmoidInp(diopiContextHandle_t ctx,
                                        diopiTensorHandle_t input) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
  DiopiTensor input_tensor(input);
...
diopiSigmoidBackward(diopiContextHandle_t ctx, diopiTensorHandle_t grad_input,
  ...
  CnnlAttribute attr;
  attr.set("mode", CNNL_ACTIVATION_SIGMOID);
  cnnl_activation_backward_internal(ctx, grad_input_tensor, grad_output_tensor,
                                    {}, output_tensor, attr);
  return diopiSuccess;
}
...
extern "C" diopiError_t diopiTanhInp(diopiContextHandle_t ctx,
                                     diopiTensorHandle_t input) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
  DiopiTensor input_tensor(input);
...
extern "C" diopiError_t diopiGelu(diopiContextHandle_t ctx,
                                  diopiTensorHandle_t out,
                                  diopiConstTensorHandle_t input,
...
extern "C" diopiError_t diopiGeluBackward(diopiContextHandle_t ctx,
                                          diopiTensorHandle_t grad_input,
                                          diopiConstTensorHandle_t grad_output,
                                          diopiConstTensorHandle_t input,
                                          const char *approximate) {
...
extern "C" diopiError_t diopiHardswish(diopiContextHandle_t ctx,
                                       diopiTensorHandle_t out,
                                       diopiConstTensorHandle_t input) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
  DiopiTensor input_tensor(input);
...
extern "C" diopiError_t diopiHardswishInp(diopiContextHandle_t ctx,
                                          diopiTensorHandle_t input) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
  DiopiTensor input_tensor(input);
...
extern "C" diopiError_t
diopiHardswishBackward(diopiContextHandle_t ctx, diopiTensorHandle_t grad_input,
                       diopiConstTensorHandle_t grad_output,
                       diopiConstTensorHandle_t output) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
  DiopiTensor grad_input_tensor(grad_input);
...
extern "C" diopiError_t diopiSilu(diopiContextHandle_t ctx,
                                  diopiTensorHandle_t out,
                                  diopiConstTensorHandle_t input) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
  DiopiTensor input_tensor(input);
...
extern "C" diopiError_t diopiSiluInp(diopiContextHandle_t ctx,
                                     diopiTensorHandle_t input) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
  DiopiTensor input_tensor(input);
...
extern "C" diopiError_t diopiSiluBackward(diopiContextHandle_t ctx,
                                          diopiTensorHandle_t grad_input,
                                          diopiConstTensorHandle_t grad_output,
                                          diopiConstTensorHandle_t output) {
  cnnlHandle_t handle = cnnlHandlePool.get(ctx);
