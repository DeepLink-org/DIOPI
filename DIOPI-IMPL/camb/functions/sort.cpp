...
diopiError_t diopiSort(diopiContextHandle_t ctx, diopiTensorHandle_t values, diopiTensorHandle_t indices, diopiConstTensorHandle_t input, int64_t dim,
    ...
    uint64_t k;
    std::vector<int64_t> input_shape = input_tensor_temp.shape();
    ...
    DIOPI_CALLCNNL(cnnlGetTopKTensorWorkspaceSize(handle, input_desc.get(), k, dim, descending, values_desc.get(), indices_desc.get(), &workspace_size));
