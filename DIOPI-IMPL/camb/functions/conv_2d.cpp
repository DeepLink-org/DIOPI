...
extern "C" diopiError_t diopiConvolution2d(diopiContextHandle_t ctx, diopiTensorHandle_t out, diopiConstTensorHandle_t input, diopiConstTensorHandle_t weight,
    ...
    DIOPI_CHECK(
        inputTensor.isContiguous() || inputTensor.isContiguous(MemoryFormat::ChannelsLast), "%s", "[diopiConvolution2d] the memory format is not supportted.");
    DIOPI_CHECK(weightTensor.isContiguous() || weightTensor.isContiguous(MemoryFormat::ChannelsLast),
                "%s",
                "[diopiConvolution2d] the memory format is not supportted.");
    DIOPI_CHECK(outputTensor.isContiguous() || outputTensor.isContiguous(MemoryFormat::ChannelsLast),
                "%s",
                "[diopiConvolution2d] the memory format is not supportted.");
...
extern "C" diopiError_t diopiConvolution2dBackward(diopiContextHandle_t ctx, diopiTensorHandle_t gradInput, diopiTensorHandle_t gradWeight,
    ...
    DIOPI_CHECK(inputTensor.isContiguous() || inputTensor.isContiguous(MemoryFormat::ChannelsLast),
                "%s",
                "[diopiConvolution2dBackward] the memory format is not supportted.");
    DIOPI_CHECK(weightTensor.isContiguous() || weightTensor.isContiguous(MemoryFormat::ChannelsLast),
                "%s",
                "[diopiConvolution2dBackward] the memory format is not supportted.");
    DIOPI_CHECK(gradOutputTensor.isContiguous() || gradOutputTensor.isContiguous(MemoryFormat::ChannelsLast),
                "%s",
                "[diopiConvolution2dBackward] the memory format is not supportted.");
    DIOPI_CHECK(gradInputTensor.isContiguous() || gradInputTensor.isContiguous(MemoryFormat::ChannelsLast),
                "%s",
                "[diopiConvolution2dBackward] the memory format is not supportted.");
    DIOPI_CHECK(gradWeightTensor.isContiguous() || gradWeightTensor.isContiguous(MemoryFormat::ChannelsLast),
                "%s",
                "[diopiConvolution2dBackward] the memory format is not supportted.");
