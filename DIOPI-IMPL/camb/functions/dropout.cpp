...
diopiError_t diopiDropout(diopiContextHandle_t ctx, diopiTensorHandle_t out,
                          ...
                          diopiConstTensorHandle_t input, double p,
                          bool train) {
  if (train) {
    ...
    // PyTorch unless p==0
    DIOPI_CHECK(((DiopiDataType::isFloatPoint(input_tensor.dtype()) || p == 0)),
                "result type Float can't be cast to the desired type");
    ...
    std::set<diopiDtype_t> supportedDtypes{
        ...
        diopi_dtype_int32, diopi_dtype_float16, diopi_dtype_float32};
    ...
    DIOPI_CALLCNNL(cnnlRandMakeMTGP32KernelState(handle, state, params,
                                                 kernel_params, rand_seed));

    // cases for dropout2d when input_shape != mask_shape
    if (input_tensor.shape() != mask_tensor.shape()) {
      DiopiTensor temp_tensor =
          ones(ctx, mask_tensor.shape(), diopi_dtype_float32);
      CnnlTensorDesc temp_desc(temp_tensor, CNNL_LAYOUT_ARRAY);

      DIOPI_CALLCNNL(cnnlFusedDropout_v2(handle, generator, temp_desc.get(),
                                         temp_tensor.data(), p, state,
      ...
      cnnlTensorDescriptor_t input_descs[] = {input_desc.get(),
                                              bcast_temp_desc.get()};
      const void *inputs[] = {input_tensor.data(), bcast_temp_tensor.data()};
      DIOPI_CALLCNNL(cnnlMulN(handle, input_descs, inputs, 2, output_desc.get(),
                              output_tensor_temp.data()))
    } else {
      // cases for dropout
      DIOPI_CALLCNNL(cnnlFusedDropout_v2(
          handle, generator, input_desc.get(), input_tensor.data(), p, state,
          mask_desc.get(), mask_tensor.data(), output_desc.get(),
          output_tensor_temp.data()));
    }
