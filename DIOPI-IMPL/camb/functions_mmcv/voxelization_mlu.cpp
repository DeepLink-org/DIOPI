...
void KernelDynamicVoxelize(cnrtDim3_t k_dim, cnrtFunctionType_t k_type, cnrtQueue_t queue, const void *points, void *coors, const float voxel_x,
                           const float voxel_y, const float voxel_z, const float coors_x_min, const float coors_y_min, const float coors_z_min,
                           const float coors_x_max, const float coors_y_max, const float coors_z_max, const int32_t grid_x, const int32_t grid_y,
                           const int32_t grid_z, const int32_t num_points, const int32_t num_features);

void KernelPoint2Voxel(cnrtDim3_t k_dim, cnrtFunctionType_t k_type, cnrtQueue_t queue, void *coors, void *point_to_pointidx, void *point_to_voxelidx,
                       const int32_t num_points, const int32_t max_points);

void KernelCalcPointsPerVoxel(cnrtDim3_t k_dim, cnrtFunctionType_t k_type, cnrtQueue_t queue, void *point_to_pointidx, void *point_to_voxelidx,
                              void *coor_to_voxelidx, void *num_points_per_voxel, void *voxel_num, const int32_t max_voxels, const int32_t num_points);

void KernelAssignVoxelsCoors(cnrtDim3_t k_dim, cnrtFunctionType_t k_type, cnrtQueue_t queue, const void *points, void *temp_coors, void *point_to_voxelidx,
                             void *coor_to_voxelidx, void *voxels, void *coors, const int32_t max_points, const int32_t num_points, const int32_t num_features);
}  // namespace camb
...
// policy function
static void policyFuncDefault(cnrtDim3_t *k_dim, cnrtFunctionType_t *k_type, const int num_points) {
    k_dim->x = impl::camb::getDeviceAttr(cnrtAttrMcorePerCluster);
    k_dim->y = MIN((num_points + k_dim->x - 1) / k_dim->x, impl::camb::getDeviceAttr(cnrtAttrClusterCount));
    k_dim->z = 1;
    *k_type = CNRT_FUNC_TYPE_UNION1;
}

// policy function
static void policyFuncCalcPointsPerVoxel(cnrtDim3_t *k_dim, cnrtFunctionType_t *k_type, const int num_points) {
    k_dim->x = 1;
    k_dim->y = 1;
    k_dim->z = 1;
...
extern "C" diopiError_t diopiHardVoxelizeMmcv(diopiContextHandle_t ctx, diopiTensorHandle_t voxels_, diopiTensorHandle_t coors_,
                                              diopiTensorHandle_t num_points_per_voxel_, diopiTensorHandle_t voxel_num_, diopiConstTensorHandle_t points_,
                                              diopiConstTensorHandle_t voxel_size_, diopiConstTensorHandle_t coors_range_, const int64_t max_points,
    ...
    std::vector<float> voxel_size_v(reinterpret_cast<float *>(voxel_size.data()), reinterpret_cast<float *>(voxel_size.data()) + voxel_size.numel());
    std::vector<float> coors_range_v(reinterpret_cast<float *>(coors_range.data()), reinterpret_cast<float *>(coors_range.data()) + coors_range.numel());
    ...
    impl::camb::KernelDynamicVoxelize(k_dim,
                                      k_type,
                                      queue,
                                      points.data(),
                                      temp_coors.data(),
    ...
    impl::camb::KernelPoint2Voxel(k_dim, k_type, queue, temp_coors.data(), point_to_pointidx.data(), point_to_voxelidx.data(), num_points, max_points);

    // calculate task dimension
    cnrtDim3_t k_dim_calc_points_per_voxel;
    cnrtFunctionType_t k_type_calc_points_per_voxel;
    policyFuncCalcPointsPerVoxel(&k_dim_calc_points_per_voxel, &k_type_calc_points_per_voxel, num_points);
    ...
    impl::camb::KernelCalcPointsPerVoxel(k_dim_calc_points_per_voxel,
                                         ...
                                         queue,
                                         point_to_pointidx.data(),
                                         point_to_voxelidx.data(),
                                         coor_to_voxelidx.data(),
                                         num_points_per_voxel.data(),
                                         voxel_num_tmp.data(),
                                         max_voxels,
                                         num_points);

    // 4. copy point features and coors of each voxels to voxels
    impl::camb::KernelAssignVoxelsCoors(k_dim,
                                        k_type,
                                        queue,
                                        points.data(),
                                        temp_coors.data(),
